...
/**
 * Editor de disponibilidad reutilizable.
 * Se usa tanto en /profesor/disponibilidad (página dedicada)
 * como dentro de /profesor/perfil (como bloque integrado).
 */
export default function DisponibilidadEditor({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-3 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2">
        <Info className="w-4 h-4 text-blue-500 flex-shrink-0" />
        <div className="flex items-center gap-3 text-xs text-gray-600 flex-wrap">
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 rounded bg-blue-100 border border-blue-200 inline-block" /> Disponible
          </span>
          <span className="text-gray-400">Clic o arrastra para seleccionar rangos</span>
        </div>
      </div>

      {loading ? (
        <div className={`${compact ? "h-48" : "h-72"} bg-white border border-gray-100 rounded-2xl animate-pulse`} />
      ) : (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 overflow-x-auto">
