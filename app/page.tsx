...
const TESTIMONIOS = [
  { texto: "Encontré a mi profesora de cálculo en 5 minutos y aprobé el examen con 17. ProfeLink cambió mi semestre.", nombre: "Luis Paredes",    carrera: "Estudiante PUCP",           foto: "https://randomuser.me/api/portraits/men/36.jpg",   nota: 5 },
  { texto: "Desde que me uní tengo 4 alumnos fijos semanales. La plataforma es súper fácil de usar y el pago llega puntual.", nombre: "María García", carrera: "Profesora verificada",    foto: "https://randomuser.me/api/portraits/women/44.jpg", nota: 5 },
  { texto: "Clase de inglés con Valentina fue increíble. Ya hablo con fluidez en mis reuniones de trabajo internacionales.", nombre: "Sofía Ríos",    carrera: "Estudiante UPC",           foto: "https://randomuser.me/api/portraits/women/67.jpg", nota: 5 },
];

const STATS = [
  { val: "500+",   label: "Profesores",         icon: Users       },
  { val: "4.9★",   label: "Rating promedio",    icon: Star        },
  { val: "10k+",   label: "Sesiones realizadas", icon: BookOpen    },
  { val: "98%",    label: "Satisfacción",        icon: Award       },
];

const FEATURES = [
  { icon: Shield,   color: "bg-indigo-100 text-indigo-600",  title: "Docentes verificados",   desc: "Cada profesor pasa por revisión manual antes de publicar su perfil en ProfeLink." },
  { icon: Star,     color: "bg-amber-100 text-amber-600",    title: "Reseñas auténticas",     desc: "Solo estudiantes con sesiones completadas pueden dejar reseñas. Cero falsas." },
  { icon: Clock,    color: "bg-emerald-100 text-emerald-600",title: "Agenda flexible",        desc: "Horarios según disponibilidad del profesor. Virtual o presencial donde quieras." },
  { icon: Zap,      color: "bg-violet-100 text-violet-600",  title: "Reserva en segundos",    desc: "Sin trámites. Elige horario, paga con Yape o tarjeta y confirma al instante." },
...
export default function HomePage() {
  return (
    <div className="min-h-screen bg-brand-bg overflow-x-hidden font-sans">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 glass border-b border-white/40 shadow-elev-1">
        <div className="max-w-6xl mx-auto px-5 py-3.5 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-glow-indigo/30 shadow-lg">
          ...
          <div className="hidden md:flex items-center gap-6">
            <Link href="/profesores" className="text-sm font-medium text-gray-600 hover:text-indigo-600 transition-colors">Profesores</Link>
            <Link href="/login"      className="text-sm font-medium text-gray-600 hover:text-indigo-600 transition-colors">Iniciar sesión</Link>
            <Link href="/register"   className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all shadow-elev-2 hover:shadow-glow-indigo/30 hover:-translate-y-0.5 active:translate-y-0">
              Comenzar gratis
            </Link>
          </div>
          <Link href="/register" className="md:hidden bg-indigo-600 text-white text-sm font-semibold px-4 py-2 rounded-xl">Comenzar</Link>
        </div>
      </nav>

      {/* ──────────────────────────────── HERO ───────────────────────────────── */}
      <section className="relative mesh-gradient pt-32 pb-20 px-5 overflow-hidden min-h-screen flex items-center">
        {/* Dot grid overlay */}
        ...
        <div className="absolute top-1/2 left-1/2 w-64 h-64 bg-emerald-500/10 rounded-full filter blur-3xl animate-blob" style={{animationDelay:"1.5s"}} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <p className="text-white/70 text-lg leading-relaxed mb-8 max-w-md animate-fade-up stagger-2">
                Asesorías personalizadas en matemáticas, ciencias, idiomas y más. Encuentra tu profesor ideal en minutos.
              </p>

              <div className="flex flex-col sm:flex-row gap-3 mb-10 animate-fade-up stagger-3">
                <Link href="/profesores" className="inline-flex items-center justify-center gap-2 bg-white text-indigo-700 font-bold px-7 py-4 rounded-2xl hover:bg-indigo-50 transition-all shadow-elev-4 hover:-translate-y-0.5 active:translate-y-0 text-base">
                  Buscar profesores <ArrowRight className="w-4 h-4" />
                </Link>
                <Link href="/register" className="inline-flex items-center justify-center gap-2 border-2 border-white/30 text-white font-semibold px-7 py-4 rounded-2xl hover:bg-white/10 transition-all text-base">
                  Soy profesor →
                </Link>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3 animate-fade-up stagger-4">
                {STATS.map((s) => (
                  <div key={s.label} className="glass rounded-2xl p-3 text-center border-white/20">
                    <p className="font-heading font-extrabold text-xl text-white">{s.val}</p>
            ...
            <div className="hidden lg:block relative h-[500px]">
              {/* Card 1 — grande */}
              <div className="absolute top-0 right-0 w-64 glass rounded-3xl p-4 animate-float shadow-elev-4">
              ...
              {/* Card 2 — medio izquierda */}
              <div className="absolute top-28 left-0 w-52 glass rounded-3xl p-4 animate-float-2 shadow-elev-3" style={{animationDelay:"1s"}}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex mt-0.5">
                      {[1,2,3,4,5].map(n => <Star key={n} className="w-2.5 h-2.5 fill-amber-400 text-amber-400" />)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Rating badge */}
              <div className="absolute bottom-32 left-8 glass rounded-2xl px-4 py-3 shadow-elev-3 animate-float" style={{animationDelay:"2s"}}>
                <div className="text-center">
                  <p className="font-heading font-extrabold text-2xl text-brand-text">4.9</p>
              ...
              {/* Sesión confirmada badge */}
              <div className="absolute bottom-16 right-8 bg-gradient-to-r from-emerald-500 to-emerald-400 rounded-2xl px-4 py-3 shadow-glow-emerald animate-float-2" style={{animationDelay:"0.5s"}}>
              ...
              {/* Card 3 - bottom right */}
              <div className="absolute bottom-0 right-4 w-56 glass rounded-3xl p-4 shadow-elev-3 animate-float" style={{animationDelay:"1.5s"}}>
      ...
      <section className="py-20 px-5 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="font-heading font-extrabold text-4xl md:text-5xl text-brand-text mb-3">
              Los mejor calificados
            </h2>
            <p className="text-gray-500 max-w-md mx-auto">Todos verificados con reseñas auténticas de estudiantes reales.</p>
          </div>

          {/* Bento grid */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
            {TOP_PROFES.map((p, i) => (
              <Link key={p.nombre} href="/profesores" className={`group bento p-4 card-lift elev-1 hover:elev-3 ${i === 0 ? "col-span-2 md:col-span-1 lg:col-span-2 lg:row-span-1" : ""}`}>
                <div className="flex items-center gap-3 mb-3">
          ...
          <div className="text-center">
            <Link href="/profesores" className="inline-flex items-center gap-2 text-indigo-600 font-semibold hover:gap-3 transition-all group">
      ...
      {/* ───────────────────────────── CÓMO FUNCIONA ─────────────────────────── */}
      <section className="py-20 px-5 bg-brand-bg relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative">
          <div className="text-center mb-14">
            ...
            <h2 className="font-heading font-extrabold text-4xl md:text-5xl text-brand-text">Empieza en 3 pasos</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
            {/* Línea conectora */}
            <div className="hidden md:block absolute top-16 left-[22%] right-[22%] h-px bg-gradient-to-r from-indigo-200 via-violet-300 to-emerald-200" />

            {PASOS.map((p, i) => (
              <div key={p.num} className="bento p-7 text-center relative elev-2 hover:elev-4 transition-all duration-300 hover:-translate-y-1">
                <span className="absolute top-4 right-4 font-heading font-black text-4xl text-indigo-50 select-none">{p.num}</span>
                <div className="w-16 h-16 bg-gradient-to-br from-indigo-100 to-violet-100 rounded-2xl flex items-center justify-center mx-auto mb-4 text-3xl">
                  {p.icon}
                </div>
      ...
      <section className="py-20 px-5 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="font-heading font-extrabold text-4xl md:text-5xl text-brand-text">
      ...
      <section className="py-20 px-5 bg-brand-bg relative overflow-hidden">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="font-heading font-extrabold text-4xl md:text-5xl text-brand-text">Lo que dicen nuestros usuarios</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {TESTIMONIOS.map((t, i) => (
              <div key={t.nombre} className={`bento p-6 elev-2 hover:elev-4 transition-all duration-300 hover:-translate-y-1 ${i === 1 ? "lg:mt-6" : ""}`}>
                {/* Stars */}
      ...
      <section className="py-16 px-5">
        <div className="max-w-4xl mx-auto">
          <div className="relative mesh-gradient rounded-4xl p-12 text-center overflow-hidden shadow-elev-4">
            ...
            <div className="relative z-10">
              ...
              <h2 className="font-heading font-extrabold text-3xl md:text-5xl text-white mb-4 leading-tight">
                ¿Eres profesor particular?
              </h2>
              <p className="text-white/70 text-lg mb-8 max-w-xl mx-auto">
                Publica tu perfil gratis, configura tu disponibilidad y empieza a recibir estudiantes. Ganas el <strong className="text-emerald-400">78%</strong> por cada sesión completada.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Link href="/register" className="inline-flex items-center gap-2 bg-white text-indigo-700 font-bold px-8 py-4 rounded-2xl hover:bg-indigo-50 transition-all shadow-elev-4 hover:-translate-y-0.5 text-base">
                  Crear mi perfil gratis <ArrowRight className="w-4 h-4" />
                </Link>
                <Link href="/profesores" className="inline-flex items-center gap-2 border-2 border-white/30 text-white font-semibold px-8 py-4 rounded-2xl hover:bg-white/10 transition-all text-base">
                  Ver profesores →
                </Link>
              </div>
            </div>
      ...
      {/* ─────────────────────────────── FOOTER ──────────────────────────────── */}
      <footer className="border-t border-brand-border py-10 px-5 bg-white">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-5">
          ...
          <p className="text-sm text-gray-400">© 2025 ProfeLink · Plataforma de asesorías académicas en Perú</p>
